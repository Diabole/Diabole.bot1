...
async def loop(self: BaseBot, user: User, message: str) -> None:
    ...
    async def loop_emote(self: BaseBot, user: User, emote_name: str) -> None:
        ...
        if user_position == None:
            await self.highrise.send_whisper(user.id,f"🚫🔄 @{user.username} Чтобы остановить цикл, просто пройдите 🔄🚫")
            return
        await self.highrise.send_whisper(user.id,f"👯🏻‍♂️🔄 @{user.username} Вы в петле   : {emote_name} 👯🏻‍♂️🔄")
        await self.highrise.send_whisper(user.id,f"🚫🔄 @{user.username} Чтобы остановить цикл, просто идите пешком  🔄🚫")
