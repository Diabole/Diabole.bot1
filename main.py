...
class Bot(BaseBot):
    ...
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        ...
        await self.highrise.send_whisper(user.id,f"❤️[Купить бота] Позвоните [@ShoKytoo] лично. ")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username in ["Misudzu","Diabole","__tvoy__"]:
              parts = message.split(" ")
              if len(parts) != 2:
        ...
        if message.lower().startswith("!tipme ") and user.username== "fufishere":
                try:
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username in ["Misudzu","Diabole","__tvoy__"]:            await self.teleporter(message)
        ...
        if        message.startswith("Help") or      message.startswith("/help") or      message.startswith("!help") or message.startswith("help"):
            await self.highrise.chat(f"!emote list | /peoples  | /marry me? | /play /fish /userinfo @ | !emoteall | !tele @ | !summon @ | !kick @ | !tele z,y,x | !tele @ z,y,x | ")
            await self.highrise.chat(f"!fit [1-11] | [Emote] All | !emote all [Emote] | !tipall [amount] | !tipme [amount]")        
        ...
        if              message.startswith("Fairyfloat All") or                              message.startswith("/emote all fairyfloat") or       message.startswith("!emote all fairyfloat"):
          if user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 1" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 2" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 3"  and user.username in ["Misudzu","Diabole","__tvoy__"]:
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
        ...
        if message == "!fit 4" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 5" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 6" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 7" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 8" and user.username in ["Misudzu","Diabole","__tvoy__"]:
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
        ...
        if message == "!fit 9" and user.username in ["Misudzu","Diabole","__tvoy__"]:
        ...
        if message == "!fit 10" and user.username in ["Misudzu","Diabole","__tvoy__"]:
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
        ...
        if message == "!fit 11" and user.username in ["Misudzu","Diabole","__tvoy__"]:
...
Item(type='clothing', amount=1, id='bag-n_registrationavatars2023furrytail', account_bound=False, active_palette=-1),
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"🦋До свидания  [{user.username}]! ")
        await self.highrise.send_emote("hcc-jetpack")